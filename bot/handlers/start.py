...
Preserve only the pose from Photo 1, not the proportions.
...
One piece garment covering both torso and legs such as dress jumpsuit overall
Apply as is.
Do not add any pants skirts or shorts.
Footwear and subtle accessories only.

Only an upper garment such as hoodie coat jacket blazer shirt sweater
Generate suitable bottoms and footwear.
Colors must complement the item but cannot duplicate it unless it is a matching set.

Only a bottom garment such as pants skirt shorts leggings
Generate a suitable top and footwear. The top must be a different color than the bottom.

A coordinated set such as костюм двойка tracksuit matching top and bottom
Use the set as the full outfit. Do not add any extra layers. Footwear only.

Preserve the scene and pose from Photo 1. This rule overrides all other outfit logic.

Footwear only
Generate a full outfit including top and bottom matching the shoe style.

Accessories only such as bag belt hat jewelry scarf socks gloves
Generate a complete outfit that fits the accessory style. Do not hide the accessory.

Color rule
The product from Photo 2 must preserve its exact real color with no hue shift no saturation change and no lightening or darkening. Generated clothing must harmonize but cannot copy the product color unless it is a matching set.

Model and scene consistency
Do not change the face skin texture facial features body shape proportions pose gesture camera angle background or lighting. Scene must match Photo 1 entirely. Resolution may be enhanced but content must stay unchanged.

Additional parameters
Garment length: {Длина изделия} The AI must always ignore any garment length visible in Photo 1 and must strictly follow the length specified here.
