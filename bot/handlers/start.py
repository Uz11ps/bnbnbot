...
Do not copy or use any clothing or style elements from Photo 1.
Do not copy or use the length, silhouette, cut or proportions of any garment from Photo 1.
Treat the model as completely nude with zero clothing context.

Use Photo 2 only for the clothing items. Reproduce the garment with absolute accuracy including fabric, texture, stitching, material shine, seams, proportions and structure.
Do not modify or reinterpret the item.
Dress the model from Photo 1 in the item from Photo 2 while keeping the exact pose, expression, lighting and background.

OUTFIT LOGIC
The product from Photo 2 is always the main item and must remain fully visible.
Ignore all outfit, footwear and colors from Photo 1.
Ignore any garment length that appears on Photo 1.
All additional clothing must be newly generated by the AI.

One piece garment covering both torso and legs such as dress jumpsuit overall: Apply as is. Do not add any pants skirts or shorts. Footwear and subtle accessories only.
Only an upper garment such as hoodie coat jacket blazer shirt sweater: Generate suitable bottoms and footwear. Colors must complement the item but cannot duplicate it unless it is a matching set.
Only a bottom garment such as pants skirt shorts leggings: Generate a suitable top and footwear. The top must be a different color than the bottom.
A coordinated set such as костюм двойка tracksuit matching top and bottom: Use the set as the full outfit. Do not add any extra layers. Footwear only.

Preserve the scene and pose from Photo 1. This rule overrides all other outfit logic.
Footwear only: Generate a full outfit including top and bottom matching the shoe style.
Accessories only such as bag belt hat jewelry scarf socks gloves: Generate a complete outfit that fits the accessory style. Do not hide the accessory.

Color rule: The product from Photo 2 must preserve its exact real color with no hue shift no saturation change and no lightening or darkening. Generated clothing must harmonize but cannot copy the product color unless it is a matching set.
Model and scene consistency: Do not change the face skin texture facial features body shape proportions pose gesture camera angle background or lighting. Scene must match Photo 1 entirely. Resolution may be enhanced but content must stay unchanged.

Additional parameters:
Garment length: {длина изделия}. Sleeve length: {длина рукав}.
...
1. Product accuracy from PHOTO 2
2. Removal of all foreign products from PHOTO 1
3. Background reconstruction from PHOTO 1
If any conflict exists — product accuracy and cleanup always win.

1️⃣ BACKGROUND USAGE RULE (NON-CREATIVE)
PHOTO 1 is used ONLY as a reference for: surface, materials, textures, lighting, perspective, composition.
This is RECONSTRUCTION, not inspiration.
❌ No stylization, No redesign, No simplification, No “similar style”. Recreate the background as close as technically possible.

🚨 2️⃣ ABSOLUTE BACKGROUND PURGE (HIGHEST PRIORITY)
If PHOTO 1 contains ANY foreign products (clothing, bags, watches, textiles, etc.), they MUST BE COMPLETELY REMOVED.
❌ They must NOT remain, be blurred, or be replaced. They must be fully erased.
2.1️⃣ BACKGROUND REBUILD AFTER REMOVAL: Rebuild missing areas using the same material, texture, and lighting from PHOTO 1.

🔒 4️⃣ PRODUCT RECONSTRUCTION (ABSOLUTE PRODUCT FIDELITY)
PHOTO 2 is the ONLY SOURCE OF TRUTH for the product.
Reproduce EXACTLY: shape, silhouette, neckline, seams, stitching, texture, and volume.
4.2️⃣ COLOR & MATERIAL LOCK (ABSOLUTE): Color must match EXACTLY. Trust PHOTO 2 for color and material.

6️⃣ SCALE, PLACEMENT & PHYSICS: Correct real-world scale and perspective. Natural contact shadows.
🎯 FINAL GOAL: Produce a photorealistic commercial image where background is from PHOTO 1 (cleaned) and product is exact from PHOTO 2."""
        prompt_filled = apply_replacements(base)
